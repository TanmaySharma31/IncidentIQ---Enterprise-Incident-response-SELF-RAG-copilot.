...
def _memory_text(state: RAGState, limit: int = 4) -> str:
    ...
    return "\n\n".join(items[-2:]) if items else "No previous conversation context."
...
def grade_relevance(state: RAGState):
    ...
    for d in state.get("docs", []):
        try:
            decision = grader.invoke(prompt.format_messages(question=state["question"], document=d.page_content[:get_settings().max_document_chars]))
...
def web_search(state: RAGState):
    ...
    response = client.search(query=q, search_depth="basic", max_results=3, include_answer=False)
    docs = []
    for r in response.get("results", []):
        content = r.get("content", "")[:get_settings().max_document_chars]
