const THREAD_KEY = 'incidentiq_thread_id';
...
function addMessage(role, html, meta=''){
  ...
  const avatar=role==='assistant'?'<div class="avatar">I</div>':'';
  const label=role==='assistant'?'INCIDENTIQ':'ON-CALL ENGINEER';
...
function loadingMarkup(){return '<span class="thinking">Running IncidentIQ <i></i><i></i><i></i></span>';}
...
async function ask(){
  ...
  try{
    ...
    const data=await r.json(); if(!r.ok){
      if(r.status===429) throw new Error('Gemini usage limit reached. Please try again after the quota resets.');
      throw new Error(data.detail||'IncidentIQ could not complete the request.');
    }
    loading.remove();
  ...
  }catch(e){loading.remove();addMessage('assistant',esc(e.message));}
...
upload.addEventListener('click',async()=>{
  ...
  try{
    ...
    if(!r.ok){
      if(r.status===429) throw new Error('Gemini usage limit reached. Document indexing is temporarily unavailable.');
      throw new Error(d.detail||'IncidentIQ could not index the document.');
    }
...
newSessionBtn?.addEventListener('click',()=>{
  ...
  localStorage.setItem(THREAD_KEY, threadId);
  renderSession();
  messages.innerHTML = `<div class="message assistant"><div class="avatar">I</div><div class="message-body"><div class="message-label">INCIDENTIQ</div><div class="bubble intro">New incident memory session started. Describe the production issue and I’ll build context across your follow-up questions.</div></div></div>`;
